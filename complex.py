def Insert_Numbers():
    ...
    user_komplex1 = input('Вставьте первое комплексное число: ')
    user_komplex2 = input('Вставьте второе комплексное число: ')
    operation = input('Что ты хочешь с этим делать? (+, -, *, /)')
...
def Repeat_Or_No():
    ...
    while user_choice != 'Y' or user_choice != 'N':
        user_choice = input('Хотите продолжить работу с комплексными числами? (Да/Нет)')
        if user_choice == 'Нет':
            return False
        elif user_choice == 'Да':
            return True
        else:
            print('Незаконный ответ! Хотите продолжить работу с комплексными числами? (Да/Нет)')